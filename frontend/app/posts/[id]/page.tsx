...
export default function PostDetailPage({ params }: { params: { id: string } }) {
  ...
  async function comment(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const form = new FormData(e.currentTarget); try { await api(`/posts/${params.id}/comments`, { method: 'POST', body: JSON.stringify({ content: form.get('content') }) }); setMessage('评论已进入审核'); e.currentTarget.reset(); } catch (err) { setMessage(err instanceof Error ? err.message : '失败'); } }
