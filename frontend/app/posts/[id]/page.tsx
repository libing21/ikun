...
export default function PostDetailPage({ params }: { params: { id: string } }) {
  ...
  return <div className="space-y-4"><article className="card"><h1 className="text-3xl font-bold">{post.title}</h1><p className="mt-4 whitespace-pre-wrap text-slate-600">{post.content}</p><div className="mt-4 flex gap-3"><button onClick={() => api(`/posts/${params.id}/like`, { method: 'POST' })}>点赞</button><button onClick={() => api(`/posts/${params.id}/favorite`, { method: 'POST' })}>收藏</button><button onClick={report}>举报</button></div></article><section className="card space-y-3"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">评论</h2><span className="text-sm text-slate-500">共 {comments.length} 条</span></div>{comments.map((c) => <p key={c.id} className="rounded-2xl bg-white/80 p-3 text-sm text-slate-700 shadow-sm">{c.content}</p>)}<form onSubmit={comment} className="flex gap-2"><input name="content" placeholder="写下你的评论，发布后立即展示" className="flex-1" required /><button>评论</button></form>{message && <p className="text-sm text-fuchsia-600">{message}</p>}</section></div>;
}
