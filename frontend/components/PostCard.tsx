  ...
  return (
    <article className="card space-y-3">
      ...
      <div className="flex flex-wrap gap-4 text-xs text-slate-500">
        ...
        <button type="button" onClick={toggleLike} className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs shadow-sm ${liked ? 'bg-rose-50 text-rose-600' : 'bg-white text-slate-500'}`}>
          <svg viewBox="0 0 24 24" className={`h-4 w-4 ${liked ? 'fill-rose-500 stroke-rose-500' : 'fill-none stroke-current'}`} strokeWidth="2">
